...
## 用户种类
**未登录**
用户无法建立单词卡或文件夹，他们可以临时访问到公共文件夹，但是并不会保存到他们的桌面，公开文件夹会短暂存在于它们的桌面，在刷新之后消失

**已登陆**
可以在桌面上保存单词卡和文件夹，或者在桌面建立访问到公共文件夹的文件夹接口

**用户数据备注**
现在程序还处于开发测试阶段，不需要在意新老用户的数据，现在所有的数据都只是测试数据
...
**使用建议**: 在修改代码前，先查阅相关文档了解模块的详细实现和依赖关系，避免破坏现有功能。尽量去复用或改造已有的函数
